...
export default function App() {
  ...
  const runTournament = useCallback(() => {
    ...
    const teamOfRoundLocal = {}; const playerOfWeekLocal = {};
    ...
    for (const g of Object.keys(groups)) {
      ...
      for (let w = 0; w < sched.length; w++) {
        ...
        for (const p of sched[w]) {
          ...
          const savesA = keeperSaves(gB), savesB = keeperSaves(gA);
          gkSaves[A.goalie.name] = (gkSaves[A.goalie.name] || 0) + savesA; gkSaves[B.goalie.name] = (gkSaves[B.goalie.name] || 0) + savesB;
        ...
        roundRatings.sort((x, y) => y.rating - x.rating); teamOfRoundLocal[roundTag] = roundRatings.slice(0, 11); if (best) playerOfWeekLocal[week - 1] = best;
      }
    }
    ...
    const [fa, fb] = winnersSF; const A = teamMap[fa], B = teamMap[fb]; const [gA, gB] = simulateScore(A, B, goalEnv, 0); const sA = starLine(A, gA), sB = starLine(B, gB);
    const finalSummary = makeSummary(A, B, gA, gB, undefined, [`${sA.who} ${sA.goals}G/${sA.assists}A`, `${sB.who} ${sB.goals}G/${sB.assists}A`]);
    all.push({ id: `Final-${A.name}-${B.name}`, round: 'Final', week, home: A.name, away: B.name, homeGoals: gA, awayGoals: gB, summary: finalSummary, starInvolvement: { [sA.who]: { goals: sA.goals, assists: sA.assists, rating: sA.rating, team: A.name }, [sB.who]: { goals: sB.goals, assists: sB.assists, rating: sB.rating, team: B.name } } });
    ...
    for (const m of all) {
      for (const [p, s] of Object.entries(m.starInvolvement)) {
        ...
        const t = stats[p]; t.team = s.team; t.matches++; t.goals += s.goals; t.assists += s.assists; t.ratingSum += s.rating;
      }
    }
    ...
    setTeamOfRound(teamOfRoundLocal); setPlayerOfWeek(playerOfWeekLocal);
  }, [groups, goalEnv, homeAdv, teamMap]);

  useEffect(() => {
    const handleKeys = e => { if (e.key.toLowerCase() === 'r') { randomizeGroups(); runTournament(); } if (e.key.toLowerCase() === 's') runTournament(); };
    window.addEventListener('keydown', handleKeys);
    return () => window.removeEventListener('keydown', handleKeys);
  }, [randomizeGroups, runTournament]);
